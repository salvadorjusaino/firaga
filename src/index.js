...
import { setupProgressBar } from './js/EmblaCarouselProgressBar';
import { addPrevNextBtnsClickHandlers } from './js/EmblaCarouselArrowButtons';
import { preloadImages } from "./js/Utils";
import { Loader } from "./js/Loader";
import { Scroll } from "./js/Scroll";
import { Player } from "./js/Player";
import { MusicService } from './js/MusicService';
	...
	initFeaturedArtists = () => {
		...
		if (sectionsPlaylists) {
			sectionsPlaylists.forEach((section, index) => {
				...
				const emblaApi = EmblaCarousel(viewportNode, OPTIONS, [ClassNames(), Fade(), Autoplay({ playOnInit: true, delay: 3000, stopOnInteraction: false })]);
	...
	initFeaturedAlbums = () => {
		...
		if (sectionsAlbums) {
			sectionsAlbums.forEach((section, index) => {
				...
				const emblaApi = EmblaCarousel(viewportNode, OPTIONS, [ClassNames(), WheelGesturesPlugin()])
